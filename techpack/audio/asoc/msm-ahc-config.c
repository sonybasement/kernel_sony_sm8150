		...
		uint32_t param_id, uint8_t *param, uint32_t param_len)
{
	...
	p_hdr = (struct param_hdr_v1 *)params_value;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	params_value = kzalloc(param_len + sizeof(struct param_hdr_v1), GFP_KERNEL);
	if (!params_value) {
		pr_err("%s: params memory alloc failed\n", __func__);
		return -ENOMEM;
	}
	 ...
	 * param_len must be increased with size of struct param_hdr_v1
	 * because adm_get_pp_params() does not add this size to the return
	 * buffer allocation even though that header is always included
	 * before the actual parameter payload.
	 */
	rc = adm_get_pp_params(AHC_PORT_ID,
			ahc_copp_idx,
			module_id,
			(struct mem_mapping_hdr *)&param_id,
			(struct param_hdr_v3 *)(param_len + sizeof(struct param_hdr_v1)),
			params_value);
	if (rc) {
